...
let home = Vue.component('home', {
	...
	methods: {
		...
		adjust_exchange: async function() {
			try {
				let result = await execute('update-exchange_rate', {id: 2} );
	...
	template: `
		<v-container>
			...
			<v-row>
				...
				<v-col>
				<v-card color="grey lighten-4">
					<v-card-title class="text-h5">DOLAR BCV HOY</v-card-title>
					<v-card-text class="text-h4">{{exchange_rate}} {{currency_default.symbol}} </v-card-text>
				</v-card>
				</v-col>
			</v-row>
			...
			<v-card color="grey lighten-4" class="pb-2" elevation="5" @click="openWidgetInvoiceDialog">
			...
			<v-col>
			<v-card color="grey lighten-4" class="pb-2" elevation="5" @click="toPurchases">
			...
			<v-col>
			<v-card color="grey lighten-4" class="pb-2" elevation="5" @click="toClients">
